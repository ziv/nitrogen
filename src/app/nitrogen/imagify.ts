...
export function toSvg(node: HTMLElement, options?: ImagifyOptions) {
    ...
    for (const [org, cloned] of pair(node, copy)) {
        if (org instanceof Element) {
            const computed = getComputedStyle(org) as CSSStyleDeclaration & Iterable<string>;
    ...
    const raw = new XMLSerializer()
        ...
        .replace(/#/g, '%23')
        .replace(/\n/g, '%0A');
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${width * sizing}" height="${height * sizing}"><foreignObject x="0" y="0" width="100%" height="100%">${raw}</foreignObject></svg>`;
